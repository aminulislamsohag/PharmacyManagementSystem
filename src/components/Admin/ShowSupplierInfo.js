...
import { fetchSuppliersData, updateSupplierData, deleteSupplierData } from '../../utils/api';
...
const ShowSupplierInfo = () => {
  ...
  const handleDeleteConfirmation = (supplier) => {
    setSelectedSupplier(supplier); // Set the selected supplier to be deleted
    setShowDeleteModal(true); // Open delete confirmation modal
  };

  const handleDelete = async () => {
    try {
      await deleteSupplierData(selectedSupplier.id); // Call the API to delete the supplier
      setSuppliers((prevSuppliers) =>
        prevSuppliers.filter((supplier) => supplier.id !== selectedSupplier.id) // Remove supplier from state
      );
  ...
  return (
    <div className="supplier-info-container">
          ...
          {suppliers.map((supplier, index) => (
            <tr key={supplier.id}>
              ...
              <td>
                ...
                <Button variant="danger" onClick={() => handleDeleteConfirmation(supplier)}>Delete</Button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)}>
        ...
        <Modal.Body>
          <p>Are you sure you want to delete <strong>{selectedSupplier.suppliername}</strong>?</p>
        </Modal.Body>
        <Modal.Footer>
